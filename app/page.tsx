...
function Logo() {
  return (
    <div className="brand" aria-label="The Feud">
      <span className="brand-work">THE</span>
...
export default function Home() {
  ...
  const [gameRounds, setGameRounds] = useState<Round[]>(CATEGORY_ROUNDS.work);
    ...
    return (
      <main className="setup-shell">
        ...
        <section className="setup-intro">
          <p className="eyebrow">ONE BOARD. SIX WAYS TO PLAY.</p>
          <h1>Choose your feud.<br /><em>Own the board.</em></h1>
          <p className="intro-copy">
            Pick a category, add everyone who is playing, and let the draw create
            two balanced teams before the showdown begins.
          </p>
        </section>
        <section className="category-picker" aria-labelledby="category-heading">
          <div className="category-heading">
            <div>
              <span>STEP 1</span>
              <h2 id="category-heading">Choose a game category</h2>
            </div>
            <b>10 ROUNDS EACH</b>
          </div>
        ...
        <div className="team-draw-label"><span>STEP 3</span> REVIEW THE DRAW</div>
        ...
        <div className="setup-actions">
          <button
            ...
            onClick={() => {
              setGameRounds(shuffleRounds(CATEGORY_ROUNDS[selectedCategory]));
              setTimerRunning(false);
              setTimeLeft(ANSWER_TIME_LIMIT);
              setStarted(true);
              setFeedback(
          ...
          <p>
            {!teamsDrawn
              ...
              : `${category.name} / ${roster.length} players / ${teams[0].players.length} vs ${teams[1].players.length} / ${CATEGORY_ROUNDS[selectedCategory].length} rounds`}
          </p>
        </div>
        <SiteFooter />
      </main>
  ...
  return (
    <main className="game-shell">
      <header className="game-header">
        <Logo />
        <div className="round-label"><small>{category.name}</small>ROUND {roundIndex + 1} <span>OF {gameRounds.length}</span></div>
